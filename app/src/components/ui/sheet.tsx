...
function SheetOverlay({ className, ...props }: SheetPrimitive.Backdrop.Props) {
  return (
    <SheetPrimitive.Backdrop
      ...
      className={cn(
        "fixed inset-0 z-50 bg-black/15 transition-opacity duration-200 ease-out data-ending-style:opacity-0 data-starting-style:opacity-0 supports-backdrop-filter:backdrop-blur-xs",
        className,
      )}
      {...props}
    />
  ...
  return (
    <SheetPortal>
      ...
      <SheetPrimitive.Popup
        ...
        data-side={side}
        className={cn(
          "fixed z-50 flex flex-col gap-4 bg-popover/96 bg-clip-padding text-sm text-popover-foreground shadow-lg backdrop-blur-xl transition-[transform,opacity] duration-[250ms] ease-[cubic-bezier(.32,.72,0,1)] data-ending-style:opacity-0 data-starting-style:opacity-0",
