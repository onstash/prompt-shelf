...
app.post('/api/templates', async (c) => {
  ...
  if (!input.title?.trim() || !input.body?.trim()) return c.json({ error: 'Title and prompt body are required' }, 422);
  const created = { ...template, id: input.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, ''), title: input.title.trim(), description: input.description?.trim() ?? '', category: input.category?.trim() ?? 'General', body: input.body, fields: input.fields ?? [], version: 1 };
